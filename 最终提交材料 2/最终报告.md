...
The final CI evidence was captured from the public GitHub Actions pages after commit `f397240` was pushed to `main`. The overview below shows the five project workflows passing against the same final evidence commit.

![E18 - Final GitHub Actions overview with five successful workflows](assets/E18-github-actions-final-overview.png)
...
| --- | --- |
| ![E19 - Android CI final successful run](assets/E19-android-ci-final-success.png) | ![E20 - Backend CI final successful run](assets/E20-backend-ci-final-success.png) |
...
The failed Trivy run is retained deliberately as remediation evidence, not as the final project state. Commit `c80bdd4` exposed real HIGH/CRITICAL dependency findings in both filesystem and image scans. Dependencies and the container base were upgraded in `bb0467d`; the final rerun at `f397240` then passed both jobs and retained both reports as artifacts. This fail-fix-rescan sequence demonstrates that the security gate was enforced rather than bypassed.
...
| --- | --- | --- |
| CI/CD pipeline | Independent Android and Go backend workflows execute on repository changes | E18 overview, E19 Android run, and E20 backend run |
| Static validation | Android lint/compile checks and backend formatting, vet, test, and build checks | Successful jobs and retained quality reports in E19-E20 |
| Unit testing | Android JVM tests and Go package tests run as merge gates | Successful test jobs in E19-E20 |
...
| Git audit trail | Commit-linked failures, remediation changes, and reruns are retained | E18, E21, E24, and repository commit history |
...
All final workflows shown in E18 are green for `f397240`. One low-severity Dependabot alert may remain visible in the repository interface; the enforced release gates specifically verify that no HIGH or CRITICAL Trivy finding remains. The report therefore does not claim that every advisory of every severity is zero.
...
| Article 32, security of processing | Input validation, parameterised SQL, security headers, tests, CodeQL, Trivy, ZAP, non-root container, and health checks | CI workflows, Dockerfile, E18-E28 | Implemented for current scope |
...
| Backend CI | Verify Go formatting, vet, tests, coverage, and build | E20 and retained backend CI artifacts | Passed |
...
| E18 final workflow overview | Section 6.3.1 | Final CI/CD acceptance evidence |
| E19-E20 Android and backend runs | Section 6.3.1 | Build, static validation, unit test, and artifact evidence |
