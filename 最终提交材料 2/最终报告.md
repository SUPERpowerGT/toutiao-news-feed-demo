...
| Phase 4 — Documentation and Testing  | Month 4  | API documentation, architecture diagrams, UML artifacts, automated testing, and evidence preparation        |
| Phase 5 — DevSecOps and Delivery     | Month 5  | CI/CD security gates, remediation, final verification, recording plans, and submission preparation          |
...
| Sprint | Sprint Goal | Backlog Items | Completed | Goal Status | Completion Evidence |
| --- | --- | ---: | ---: | --- | --- |
| Sprint 0 | Build the project foundation | 4 | 4 | Achieved | Client and backend can start independently; database service is available |
| Sprint 1 | Complete core feed workflow | 4 | 4 | Achieved | First-screen feed and pagination can be demonstrated |
| Sprint 2 | Complete refresh and cache system | 4 | 4 | Achieved | Refresh, weak-network fallback, and skeleton transition can be verified |
| Sprint 3 | Improve architecture quality | 4 | 4 | Achieved | Architecture diagrams and core module boundaries are stable |
| Sprint 4 | Prepare documentation and verification | 4 | 4 | Achieved | Report artifacts, screenshots, and API logs are available |
| Sprint 5 | Complete CI/CD and final technical delivery | 4 | 4 | Achieved | Final code and five GitHub workflows are verified; recording is a submission activity |

All 24 Sprint Backlog items are recorded as complete in `evidence/sprint-backlog.csv`. All 12 user stories committed to the current release are complete in `evidence/management-product-backlog.csv`; PB-13 is labelled as a future-scope epic and was never accepted into a current Sprint.

### Planned Effort vs Actual Effort

The internship working period is recorded as **2026-03-23 to 2026-08-14**. For management reporting, the sole developer confirmed an approximate working intensity of **20 working days per month for five months**, giving **100 man-days** and **800 hours**. Planned and actual effort are both reported as approximately 800 hours. This is a retrospective self-declaration rather than a contemporaneous daily timesheet.

| Member | Role | Planned Man-Days | Actual Man-Days | Planned Hours | Actual Hours | Evidence Classification |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| Xu Ziyi | Sole Developer: Android, backend, database, testing, DevSecOps, and documentation | 100 | 100 | 800 | 800 | Approximate retrospective self-declaration; repository history supports activity, not exact hours |
...
xychart-beta
    ...
    line "Planned" [24, 20, 16, 12, 8, 0]
    line "Actual" [22, 18, 14, 10, 5, 0]
```
...
### Sole-Developer Team Management

Because this is a one-person project, team management means controlling role switching, workload, quality gates, and delivery risk rather than allocating work to invented members.

| Team Management Issue | Mitigation | Outcome |
| --- | --- | --- |
| Context switching across Android, Go, PostgreSQL, testing, and documentation | Use Sprint goals and P0/P1/future-scope priorities to limit concurrent work | Core release scope completed without claiming future features |
| No independent teammate review | Use automated tests, CodeQL, Trivy, ZAP, repeatable scripts, and visible Git evidence as quality gates | Five final GitHub workflows passed against the same evidence commit |
| Schedule pressure near final delivery | Separate technical completion from recording and packaging activities | All in-scope stories and Sprint tasks completed; recording remains a submission activity |
| Multiple Git author identities | Record all identities as belonging to Xu Ziyi | Effort is reported once for one member rather than triple-counted |
...
### 6.2.9 Final App-Demo Rehearsal Evidence

The final rehearsal exercised the running Android application against the Docker Compose Go/PostgreSQL stack. The backend now embeds a deterministic MP4 and serves it from `/media/demo-video.mp4` with the correct media type and byte-range support, removing reliance on an external media host during assessment. Backend tests verify both complete and ranged media responses.

| Live feed | Video channel | Real playback |
| --- | --- | --- |
| ![E35 - live recommendation feed](assets/E35-app-demo-live-home.png) | ![E36 - live video channel](assets/E36-app-demo-video-channel.png) | ![E37 - backend-hosted video playback](assets/E37-app-demo-video-playback.png) |

Five newer records were appended through the backend before pull-to-refresh. Deep scrolling exercised cursor-based pagination. For failure recovery, the backend container was stopped and the app was force-stopped and relaunched: the feed remained available from Room, while an uncached detail request showed an explicit Retry state. After the backend was started, Retry recovered the same detail without restarting the app.

| Refresh result | Pagination result | Offline Room feed |
| --- | --- | --- |
| ![E38 - incremental refresh](assets/E38-app-demo-refresh-result.png) | ![E39 - pagination](assets/E39-app-demo-pagination.png) | ![E40 - Room fallback](assets/E40-app-demo-offline-room-cache.png) |

| Controlled detail failure | In-place recovery |
| --- | --- |
| ![E41 - explicit error and Retry](assets/E41-app-demo-detail-error-retry.png) | ![E42 - recovered detail](assets/E42-app-demo-retry-recovered.png) |
...
### 6.3.1 Final GitHub Actions Evidence

The final CI evidence was captured from the public GitHub Actions pages after commit `900f4e5` was pushed to `main`. The overview below shows the five project workflows passing against the same final evidence commit, including GDPR control validation and the saved container-image artifact.

![E29 - Final GitHub Actions overview for the compliance and saved-image evidence commit](assets/E29-github-actions-compliance-final-overview.png)

The Android and backend workflows provide separate build-and-test gates. The Android run retained the debug APK and quality reports, while the backend run retained its test, coverage, and build evidence.

| Android CI | Backend CI |
| --- | --- |
| ![E19 - Android CI successful run](assets/E19-android-ci-final-success.png) | ![E30 - Backend CI with binary/coverage and saved-image artifacts](assets/E30-backend-ci-saved-image-artifact.png) |

The security workflow scanned both the repository filesystem and the built backend container image. CodeQL analysed both Go and Java/Kotlin. The integration workflow exercised the deployed service, executed the load test, and ran the ZAP baseline scan.

| Dependency and Container Security | SAST - CodeQL |
| --- | --- |
| ![E21 - Trivy filesystem and image scans passing](assets/E21-container-security-final-success.png) | ![E23 - CodeQL final successful run](assets/E23-codeql-final-success.png) |

![E22 - Integration, load, and DAST final successful run](assets/E22-integration-load-dast-final-success.png)

The downloadable `integration-load-test-results` artifact was opened and retained from the later successful workflow run `#8` on commit `0395302`. Its integration log records successful health, seed reset, recommendation, refresh, video-scene, cursor, input-validation, and method checks. Its ApacheBench log records 500 completed requests at concurrency 25 with 0 failures, 558.60 requests per second, 44.754 ms mean latency, 63 ms P95, 69 ms P99, and 78 ms maximum latency. E31 and E32 are readable evidence views derived from the retained logs; the original `.log` files remain the audit source. These are supplementary repeat-run values; they do not replace the separately documented 1,267.76 requests-per-second remediation benchmark because performance varies by runner and execution conditions.

| Downloaded integration-test artifact | Downloaded load-test artifact |
| --- | --- |
| ![E31 - Integration artifact checks passing](assets/E31-integration-artifact-results.png) | ![E32 - Load artifact with 500 requests and zero failures](assets/E32-load-artifact-results.png) |

GitHub's generated Dependency Graph workflow also completed successfully for the backend Go modules, providing a current dependency inventory for Dependabot analysis.

![E25 - Dependency Graph update successful](assets/E25-dependency-graph-success.png)

### 6.3.2 Security Remediation Trace

The failed Trivy run is retained deliberately as remediation evidence, not as the final project state. Commit `c80bdd4` exposed real HIGH/CRITICAL dependency findings in both filesystem and image scans. Dependencies and the container base were upgraded in `bb0467d`; final evidence commit `900f4e5` passed both jobs and retained both reports as artifacts. This fail-fix-rescan sequence demonstrates that the security gate was enforced rather than bypassed.

| Before remediation | After remediation |
| --- | --- |
| ![E24 - Trivy gate failure before dependency remediation](assets/E24-trivy-remediation-before.png) | ![E21 - Trivy gate passing after dependency remediation](assets/E21-container-security-final-success.png) |

OWASP ZAP produced a separate DAST remediation loop. At commit `bb0467d`, integration and load testing passed but the DAST gate failed because ZAP identified alerts. The API then added a valid same-origin Cross-Origin-Resource-Policy header, and rules for intentional non-storable API responses and Unix timestamps were documented in `.zap/rules.tsv`. Commit `d16f5f8` and the final evidence run passed; the retained report records 65 PASS, 0 FAIL, 0 WARN, and 2 justified IGNORE results.

| DAST before remediation | DAST after remediation |
| --- | --- |
| ![E28 - ZAP DAST gate failed after identifying alerts](assets/E28-zap-remediation-before.png) | ![E22 - Integration, load, and ZAP DAST jobs passing](assets/E22-integration-load-dast-final-success.png) |

CodeQL analysed Go and Java/Kotlin successfully in E23. A separate version-controlled Semgrep rule provides a reproducible source-code remediation loop for privacy-sensitive logging. The same rule finds one CWE-532 issue in historical commit `f397240`, where `feed_handler.go` logged `r.RemoteAddr`, and finds zero issues in the current backend after the client address was removed. E33 shows the exact source location and before/after counts; the raw JSON and summary are retained under `evidence/sast-remediation/`. This is accurately described as a historical same-rule comparison, not as an alert originally raised by CodeQL.

![E33 - Semgrep SAST client-IP logging remediation and clean rescan](assets/E33-semgrep-sast-remediation.png)

The online Semgrep job in SAST run `#15` completed the historical scan, current rescan, count/error gate, and artifact upload successfully. The retained `semgrep-sast-remediation` artifact contains both JSON reports and the generated summary.

![E34 - Online Semgrep remediation and artifact job successful](assets/E34-semgrep-sast-ci-success.png)

### 6.3.3 Container Build, Save, Inspect, and Log Evidence

The backend uses a multi-stage Go build, a minimal Alpine runtime, a non-root `app` user, and an image-level health check. Runtime inspection confirmed `uid=100(app)`, both Compose services healthy, and membership of the private `toutiao_net` bridge network. Operational logs record method, route, status code, and duration without retaining the client IP in application-level feed logs.

![E26 - Healthy containers, non-root identity, network, and runtime logs](assets/E26-container-runtime-evidence.png)

The image was also exported through `docker save`, compressed, validated, and hashed. The local archive was 7.5 MB with SHA-256 `356a7d553eb68b1356f5c718d9113baedd002d3057040f361aa21f2889e2df4f`. Backend CI repeats this process and uploads the archive plus digest as the short-retention `backend-container-image` artifact.

![E27 - Backend image inspect, save, archive validation, and digest evidence](assets/E27-saved-image-evidence.png)

![E30 - Successful Backend CI run with backend and saved-image artifacts](assets/E30-backend-ci-saved-image-artifact.png)

## 6.4 DevSecOps Evidence Matrix

The final DevSecOps implementation combines build validation, automated tests, SAST, dependency and container scanning, integration verification, load testing, DAST, retained artifacts, and a Git-backed remediation history.

| DevSecOps Area | Implemented Control | Final Evidence |
| --- | --- | --- |
| CI/CD pipeline | Independent Android and Go backend workflows execute on repository changes | E29 overview, E19 Android run, and E30 backend run |
| Static validation | Android lint/compile checks and backend formatting, vet, test, and build checks | Successful jobs and retained quality reports in E19/E30 |
| Unit testing | Android JVM tests and Go package tests run as merge gates | Successful test jobs in E19/E30 |
| Integration testing | Docker-backed PostgreSQL and backend service are started before executable API verification | Integration job in E22 and downloaded artifact E31 |
| Load and stress testing | Automated load stage sends 500 requests at concurrency 25 | Successful load job E22 and downloaded result E32 |
| SAST | CodeQL analyses Go and Java/Kotlin; Semgrep enforces a privacy-sensitive logging rule and reproduces a historical fix | E23 two-language CodeQL run, E33 before/after comparison, and raw Semgrep JSON |
| Dependency scanning | Trivy scans the repository and gates HIGH/CRITICAL findings | E24 failure followed by E21 successful remediation rerun |
| Dependency inventory | GitHub Dependency Graph records the backend Go modules for Dependabot analysis | E25 successful graph update |
| DAST | OWASP ZAP baseline scan runs against the same-origin test deployment | Successful DAST job and retained ZAP report in E22 |
| Container management | Docker Compose starts, inspects, and logs healthy PostgreSQL and backend containers | E08 and E26 |
| Image build and save | Backend CI builds the image, exports it with `docker save`, records SHA-256, and uploads an artifact | E27 and `backend-container-image` artifact |
| Image security | Backend image is built and scanned separately by Trivy | Successful `image-scan` job and `trivy-image-report` artifact in E21 |
| Git audit trail | Commit-linked failures, remediation changes, and reruns are retained | E24, E28, E29, and repository commit history |
| Infrastructure / Compliance as Code | Docker Compose, GitHub Actions, and a machine-validated GDPR control map are versioned | `docker-compose*.yml`, `.github/workflows/`, `compliance/gdpr-controls.json` |

All final workflows shown in E29 are green for `900f4e5`. One low-severity Dependabot alert remains visible in the repository interface; the enforced release gates specifically verify that no HIGH or CRITICAL Trivy finding remains. The report therefore does not claim that every advisory of every severity is zero.
...
### 6.5.1 GDPR Applicability and Compliance-as-Code

The selected regulatory framework is the **EU General Data Protection Regulation (GDPR)**. This is an applicability and control-mapping exercise, not a certification or claim of full production compliance. The current release has no login, user profile, advertising identifier, behavioural tracking, or user-activity personalisation. However, infrastructure access logs may process IP addresses, and future account or analytics features would introduce additional personal-data obligations.

| GDPR Control | Current Implementation | Evidence | Status |
| --- | --- | --- | --- |
| Article 5(1)(c), data minimisation | No account/profile tracking; application feed logs omit client IP, bodies, tokens, and credentials | `feed_handler.go`, `logging.go`, release scope | Implemented for current scope |
| Article 25, privacy by design | Authentication, social interaction, and behaviour-based personalisation remain outside the release; Room stores content without a cross-device profile | Product backlog and Android local-data layer | Implemented for current scope |
| Article 32, security of processing | Input validation, parameterised SQL, security headers, tests, CodeQL, Trivy, ZAP, non-root container, and health checks | CI workflows, Dockerfile, E18-E30 | Implemented for current scope |
| Article 5(1)(e), storage limitation | No production retention claim is made; public deployment requires log retention/deletion, consent, data-subject request, and breach-response procedures | GDPR control map and known limitations | Required before public deployment |

The machine-readable mapping is stored in `compliance/gdpr-controls.json`. `scripts/validate_compliance.sh` validates the framework name, unique control identifiers, references, evidence, and status fields, and Backend CI executes this validation. This turns the documented applicability decision into a version-controlled compliance-as-code artifact while keeping the remaining production obligations explicit.
...
| Backend CI | Verify GDPR mapping, Go formatting, vet, tests, coverage, build, and saved image | E30 and retained backend artifacts | Passed |
| Integration and load | Verify API integration and 500 requests at concurrency 25 | E22 and retained load-test results | Passed, 0 failed requests |
| SAST | Analyse Go and Java/Kotlin source with CodeQL | E23 | Passed |
| Dependency and image security | Gate HIGH/CRITICAL findings in filesystem and container image | E21 and retained Trivy reports | Passed after remediation |
| DAST | Run OWASP ZAP baseline against the integrated service | E22 and retained ZAP report | Passed: 65 PASS, 0 FAIL, 0 WARN, 2 justified IGNORE |
...
| E18 historical workflow overview | Section 6.3.1 | Earlier finalisation evidence |
| E19 Android run | Section 6.3.1 | Android build, lint, unit test, and artifact evidence |
| E21 and E24 Trivy before/after runs | Section 6.3.2 | Vulnerability discovery, remediation, and rescan evidence |
| E22 integration/load/DAST run | Section 6.3.1 | Integration, performance, and dynamic security evidence |
| E23 CodeQL run | Section 6.3.1 | Multi-language SAST evidence |
| E25 Dependency Graph run | Section 6.3.1 | Dependency inventory and Dependabot evidence |
| E26 container runtime evidence | Section 6.3.3 | Container health, inspect, non-root identity, and logs |
| E27 saved-image evidence | Section 6.3.3 | Image build, export, digest, and artifact evidence |
| E28 ZAP failure | Section 6.3.2 | DAST finding, quality gate, remediation, and rescan evidence |
| E29 final workflow overview | Section 6.3.1 | Final compliance/saved-image CI acceptance evidence |
| E30 Backend CI artifacts | Sections 6.3.1 and 6.3.3 | Coverage/binary and saved container-image artifacts |
| E31 downloaded integration artifact | Section 6.3.1 | Executable API integration checks from GitHub Actions run `#8` |
| E32 downloaded load artifact | Section 6.3.1 | ApacheBench 500/25 result with zero failed requests |
| E33 Semgrep SAST remediation | Section 6.3.2 | Historical client-IP logging finding, source remediation, and same-rule clean rescan |
| E34 online Semgrep SAST job | Section 6.3.2 | Historical scan, current rescan, enforcement gate, and artifact upload successful in GitHub Actions |
| E35-E42 app rehearsal sequence | Section 6.2.9 | Live feed, video playback, refresh, pagination, Room fallback, error, and Retry recovery |
...
## 8.4 Internal Deployment and Sponsor Evidence Boundary

The project developer reports that the system has been deployed in an internal environment of a China-based company. The company does not permit disclosure of production URLs, infrastructure identifiers, application screenshots, traffic or user data, deployment logs, or Sponsor communications because of corporate confidentiality, cybersecurity, and data-compliance requirements.

This report therefore records **internal go-live as a developer declaration**. It does not present the local demo screenshots, GitHub CI results, or generated evidence as screenshots of the confidential production environment. No independently verifiable public deployment or formal Sponsor sign-off is attached. The technical evidence in this report demonstrates that the submitted version is runnable, tested, scanned, and deployment-ready without exposing protected company information.

| Status Area | Submission Position |
| --- | --- |
| Internal go-live | Reported by the project developer |
| Public production URL or screenshot | Not disclosed under company confidentiality and compliance requirements |
| Production users, traffic, logs, and infrastructure | Not disclosed |
| Formal Sponsor sign-off | Not included in the submission materials |
| Publicly reviewable evidence | Sanitized local demo, repository history, tests, CI artifacts, and security results |
...
| E33 | Semgrep SAST remediation and rescan | DONE | One historical CWE-532 client-IP logging finding and zero current findings under the same rule |
| E34 | Online Semgrep SAST job | DONE | GitHub Actions run `#15` shows scan, rescan, evidence gate, and artifact upload successful |
| E35-E42 | Final app-demo rehearsal | DONE | Live feed, video channel/playback, refresh, pagination, Room offline fallback, error, and recovery |
