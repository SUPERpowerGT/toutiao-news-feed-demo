...
- [x] CD position is explicit and evidence-aligned: the workflows provide continuous integration, delivery-ready artifacts, and automated deployment verification on an ephemeral Compose environment. They do not automatically deploy to a persistent production environment and are not presented as production CD.
...
| 8:40-9:15 | Compose files, workflow files, compliance JSON, validation script | "Infrastructure and compliance controls are versioned as code. Docker Compose defines services, health checks, dependencies, storage, ports, and the private network. GitHub workflow files define builds, tests, scans, gates, and retained artifacts. This is continuous integration plus continuous-delivery readiness: the workflow creates delivery artifacts and automatically deploys an ephemeral Compose stack for verification, but it does not automatically deploy to a persistent production environment. The GDPR mapping is stored as machine-readable JSON and validated with jq. These files are reviewed through the same Git history, providing an auditable link between infrastructure intent, compliance decisions, execution, and final evidence." |
...
- [x] CI/CD wording accurately distinguishes delivery-ready artifacts and ephemeral deployment verification from persistent production deployment.
