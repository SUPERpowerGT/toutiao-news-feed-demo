...
| 2:25-3:05 | Show E23, E33, then E21. | "CodeQL analyses both Go and Java or Kotlin source. A version-controlled Semgrep privacy rule separately reproduces one historical client-IP logging finding and zero current findings under the same rule; I do not misattribute that remediation to CodeQL. Trivy provides the dependency and image-security gate. Its filesystem and image jobs create SARIF and report artifacts, then fail on HIGH or CRITICAL findings. The successful run publishes trivy-filesystem-report and trivy-image-report. One separate low-severity Dependabot notice remains disclosed." |
...
- Attribute the source remediation to Semgrep and the broad two-language analysis to CodeQL; do not call the Semgrep finding a CodeQL alert.
