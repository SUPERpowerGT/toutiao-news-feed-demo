...
| 4 DevSecOps | Page 36: CI/CD test and scan artifacts; containers; SAST/DAST resolution/rescan; IaC/Git; regulatory framework | CodeQL and Semgrep SAST, Trivy, ZAP remediation loops, container evidence, IaC/Git, and GDPR are implemented. SAST run `#15` and its remediation artifact are green | **Three evidence gaps:** opened unit-test/coverage result, opened final ZAP report, and an explicit CD position or sanitized persistent-deployment evidence |
...
1. Video 4: capture an opened unit-test/coverage report and an opened final ZAP report; settle the CD wording/evidence before recording.
2. Video 5: obtain sanitized Sponsor acceptance or feedback. This is the only remaining rubric item that cannot be generated from the repository.
...
4. Replace `TeamXX` and the ZIP placeholder with the actual team identifier and project title before packaging.
